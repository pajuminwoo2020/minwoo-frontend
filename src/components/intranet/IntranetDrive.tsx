...
import {
  getBoardIntranetDrives,
  getBoardIntranetDrive,
  createBoardIntranetDrive,
  deleteBoardIntranetDrive,
  updateBoardIntranetDrive,
} from 'libs/api/board';
...
const IntranetDrive = () => {
  ...
  const getPromise = getBoardIntranetDrives.bind(null, {
    params: {
  ...
  const columns: ColumnsType<TBoardDetail> = [
    ...
    },
    {
      ...
      render: (_: any, record: TBoardDetail) => (
        <Link to={`${ERoute.IntranetDrive}/${EBoardOperation.View}/${record.id}`}>
          {record.title}
        </Link>
      )
    },
    ...
    <>
      <TableHeaderWrapper>
        ...
        {boardManagementPermission &&
          <Link to={`${ERoute.IntranetDrive}/${EBoardOperation.Create}`}>
            <Button
...
export const IntranetDriveDetail = () => {
  const match = useRouteMatch(`${ERoute.IntranetDrive}/:operation/:record_id?`);
  let {operation=EBoardOperation.View, record_id} = (match?.params as RouteMatch) || {};
  const [{data, loading}] = useDataApi<TBoardDetail>(getBoardIntranetDrive.bind(null, record_id), {}, operation != EBoardOperation.Create);

  return (
	<BoardDetail
	  operation={operation}
	  pathName={ERoute.IntranetDrive}
	  promiseCreate={createBoardIntranetDrive}
	  promiseDelete={deleteBoardIntranetDrive}
	  promiseUpdate={updateBoardIntranetDrive}
	  record={data}
	/>
  );
};

IntranetDrive.defaultProps = {};

export default IntranetDrive;
