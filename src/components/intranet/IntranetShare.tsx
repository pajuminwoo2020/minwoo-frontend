...
import {
  getBoardIntranetShares,
  getBoardIntranetShare,
  createBoardIntranetShare,
  deleteBoardIntranetShare,
  updateBoardIntranetShare,
} from 'libs/api/board';
...
const IntranetShare = () => {
  ...
  const getPromise = getBoardIntranetShares.bind(null, {
    params: {
  ...
  const columns: ColumnsType<TBoardDetail> = [
    ...
    },
    {
      ...
      render: (_: any, record: TBoardDetail) => (
        <Link to={`${ERoute.IntranetShare}/${EBoardOperation.View}/${record.id}`}>
          {record.title}
        </Link>
      )
    },
    ...
    <>
      <TableHeaderWrapper>
        ...
        {boardManagementPermission &&
          <Link to={`${ERoute.IntranetShare}/${EBoardOperation.Create}`}>
            <Button
...
export const IntranetShareDetail = () => {
  const match = useRouteMatch(`${ERoute.IntranetShare}/:operation/:record_id?`);
  let {operation=EBoardOperation.View, record_id} = (match?.params as RouteMatch) || {};
  const [{data, loading}] = useDataApi<TBoardDetail>(getBoardIntranetShare.bind(null, record_id), {}, operation != EBoardOperation.Create);

  return (
	<BoardDetail
	  operation={operation}
	  pathName={ERoute.IntranetShare}
	  promiseCreate={createBoardIntranetShare}
...
IntranetShare.defaultProps = {};

export default IntranetShare;
