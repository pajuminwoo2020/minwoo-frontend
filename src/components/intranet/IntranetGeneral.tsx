...
import {
  getBoardIntranetGenerals,
  getBoardIntranetGeneral,
  createBoardIntranetGeneral,
  deleteBoardIntranetGeneral,
  updateBoardIntranetGeneral,
} from 'libs/api/board';
...
const IntranetGeneral = () => {
  ...
  const getPromise = getBoardIntranetGenerals.bind(null, {
    params: {
  ...
  const columns: ColumnsType<TBoardDetail> = [
    ...
    },
    {
      ...
      render: (_: any, record: TBoardDetail) => (
        <Link to={`${ERoute.IntranetGeneral}/${EBoardOperation.View}/${record.id}`}>
          {record.title}
        </Link>
      )
    },
    ...
    <>
      <TableHeaderWrapper>
        ...
        {boardManagementPermission &&
          <Link to={`${ERoute.IntranetGeneral}/${EBoardOperation.Create}`}>
            <Button
...
export const IntranetGeneralDetail = () => {
  const match = useRouteMatch(`${ERoute.IntranetGeneral}/:operation/:record_id?`);
  let {operation=EBoardOperation.View, record_id} = (match?.params as RouteMatch) || {};
  const [{data, loading}] = useDataApi<TBoardDetail>(getBoardIntranetGeneral.bind(null, record_id), {}, operation != EBoardOperation.Create);

  return (
	<BoardDetail
	  operation={operation}
	  pathName={ERoute.IntranetGeneral}
	  promiseCreate={createBoardIntranetGeneral}
	  promiseDelete={deleteBoardIntranetGeneral}
	  promiseUpdate={updateBoardIntranetGeneral}
	  record={data}
	/>
  );
};

IntranetGeneral.defaultProps = {};

export default IntranetGeneral;
